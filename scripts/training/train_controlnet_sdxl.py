...
from datasets.features import Image as ImageFeature #! ADDED
...
def log_validation(vae, unet, controlnet, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        transform = transforms.Compose(
            [
                transforms.Resize((args.resolution, args.resolution), interpolation=interpolation),  #! ADDED
...
def get_train_dataset(args, accelerator):
    ...
    dataset = dataset.cast_column("file_name", ImageFeature()) #! ADDED
    dataset = dataset.cast_column("conditioning_image", ImageFeature()) #! ADDED
...
def prepare_train_dataset(dataset, accelerator):
    ...
    image_transforms = transforms.Compose(
        [
            transforms.Resize((args.resolution, args.resolution), interpolation=interpolation_mode),  #! ADDED
...
# ! ADDED FUNCTIONS FOR GPU MEMORY MANAGEMENT ----------------------------------------------------------------------------

def clear_memory(*models):
    """Move given models to CPU and free memory"""
    import gc
    for model in models:
...
def restore_models(accelerator, models, weight_dtype):
    """Restore models to GPU with proper dtype"""
    for model in models:
...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt is not None:
            ...
            #! ADDED
            print("\033[91mFreeing GPU memory for final validation...\033[0m")
            print(f"GPU memory usage before clearing: {torch.cuda.memory_allocated() / 1e9:.2f} GB")

            moved_models = clear_memory(vae, unet, controlnet, text_encoder_one, text_encoder_two)

            print(f"GPU memory usage after clearing: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
